...
function presetGenerator(tree) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        yield (0, devkit_1.addDependenciesToPackageJson)(tree, {
            chalk: 'latest',
        }, {
            '@types/node': 'latest',
            typescript: 'latest',
        ...
        yield (0, devkit_1.installPackagesTask)(tree, true, undefined, 'pnpm');
        const days = [];
        for (let i = 1; i <= 25; i++) {
            days.push(`day-${i}`);
        }
...
function createSampleDataFile(tree, part, dayName) {
    const filePath = path.join(dayName, `${part}.data.sample.txt`);
    const content = '';
    tree.write(filePath, content);
}
function createActualDataFile(tree, part, dayName) {
    const filePath = path.join(dayName, `${part}.data.txt`);
    const content = '';
    tree.write(filePath, content);
}
function addUtilsFile(tree) {
...
function makeChangesToNxJson(tree) {
...
function addModuleToPackageJson(tree) {
    const packageJson = JSON.parse(tree.read('package.json').toString());
    packageJson.type = 'module';
    tree.write('package.json', JSON.stringify(packageJson, null, 2));
}
function addPrettierRc(tree) {
    const content = `{
  "singleQuote": true,
}
`;
